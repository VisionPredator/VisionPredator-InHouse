...
{
	//auto& comp = std::any_cast<std::reference_wrapper<PlayerComponent>>(entityID).get();

	auto gunID = std::any_cast<uint32_t>(entityID);

	//auto particle = comp.GetComponent<ParticleComponent>();
	//particle->IsRender = true;
	//particle->Restart = true;

	const auto& children = GetSceneManager()->GetChildEntityComps_HasComp<ParticleComponent>(gunID);
	children[0]->IsRender = true;
	children[0]->Restart = true;
}
