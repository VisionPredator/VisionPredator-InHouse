                                      ...
                                      std::shared_ptr<ResourceManager>& resourceManager, const uint32_t& width, const uint32_t& height)
{
	...
	m_StaticMeshVS = std::make_shared<VertexShader>(device, L"../../../VPGraphics/MeshVS.hlsl", -1);
	m_SkeletalMeshVS = std::make_shared<VertexShader>(device, L"../../../VPGraphics/MeshVS.hlsl", -1);
	m_GeometryPS = std::make_shared<PixelShader>(device, L"MeshDeferredGeometry");

	m_ModelTransformCB = std::make_shared<ConstantBuffer<TransformData>>();
	m_BoneTransformCB = std::make_shared<ConstantBuffer<MatrixPallete>>();
...
//{
	...
	//	m_Device->Context()->PSSetConstantBuffers(0, 1, m_ModelTransformCB->GetAddress());
	//	m_Device->Context()->PSSetConstantBuffers(1, 1, m_BoneTransformCB->GetAddress());
